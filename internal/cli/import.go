...
import (
	...
	"github.com/Benjamin-van-Heerden/memr/internal/importer"
...
func (a *app) importCommand() *cobra.Command {
	...
	cmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(out, "AGENTS.md: old harness block replaced by the memr block, %d memories added, other content kept\n", sum.Memories)
			fmt.Fprintf(out, "Docs: %d in .memr/docs/\n", sum.Docs)
			if sum.Structure {
				fmt.Fprintln(out, "Structure doc: codebase_and_structure.md moved to .memr/structure.md")
			}
			fmt.Fprintf(out, "Specs: %d open, %d archived\nTodos: %d\nWork logs: %d\n", sum.Specs, sum.Archived, sum.Todos, sum.Logs)
			if len(sum.Runnables) > 0 {
				fmt.Fprintf(out, "Runnables: %s in .memr/runnables/, from the old files, tree_dirs and runnables settings\n", strings.Join(sum.Runnables, ", "))
			}
			...
			output.Instruction(out,
				...
				"1. Show the user the result: `git status`, AGENTS.md and .memr/.",
